...
const MAX_LIMITS = { perMinute: 20, perDay: 50 }

const showLimits = computed(() => selectedProvider.value === 'openrouter')

const perMinutePercent = computed(() =>
  Math.min(100, (limits.value.perMinute / MAX_LIMITS.perMinute) * 100),
)

const perDayPercent = computed(() =>
  Math.min(100, (limits.value.perDay / MAX_LIMITS.perDay) * 100),
)
...
function handleClickOutside(e: MouseEvent) {
  if (
    providerRef.value &&
    !providerRef.value.contains(e.target as Node)
  ) {
    isProviderOpen.value = false
  }
  ...
  if (
    modelRef.value &&
    !modelRef.value.contains(e.target as Node)
  ) {
    isModelOpen.value = false
  }
}
