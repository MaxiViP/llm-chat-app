  ...
  <div class="model-select p-3 bg-gray-50 border-b border-gray-200 flex flex-col gap-2">
    ...
    <div class="flex flex-col sm:flex-row gap-3">
      ...
      <div class="relative w-full sm:w-48">
        ...
        <span class="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-gray-500">▾</span>
      </div>
    ...
    <div v-if="showLimits" class="mt-2 space-y-1 text-xs">
      <div>Лимит запросов/мин: {{ limits.perMinute }} / {{ MAX_LIMITS.perMinute }}</div>
      ...
      <div>Лимит запросов/день: {{ limits.perDay }} / {{ MAX_LIMITS.perDay }}</div>
      <div class="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
        <div
          class="h-full rounded-full transition-all duration-300"
          :style="{
...
const perMinutePercent = computed(() => Math.max(0, Math.min(100, (limits.value.perMinute / MAX_LIMITS.perMinute) * 100)))
const perDayPercent = computed(() => Math.max(0, Math.min(100, (limits.value.perDay / MAX_LIMITS.perDay) * 100)))
...
watch(() => store.provider, (val) => { selectedProvider.value = val })
