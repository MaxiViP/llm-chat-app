  ...
  <div class="chat-input flex-shrink-0 bg-white border-t border-gray-200 shadow-lg p-3 sm:p-4">
    <div class="flex flex-col sm:flex-row gap-2 sm:gap-3">
      <textarea
        ...
        @keydown.enter.ctrl.prevent.exact="send"
        @keydown.up.prevent="historyUp"
        @keydown.down.prevent="historyDown"
        :disabled="disabled"
        :placeholder="placeholderText"
        rows="2"
        class="flex-1 px-3 py-2 sm:py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 disabled:bg-gray-100 disabled:cursor-not-allowed resize-none text-sm sm:text-base transition-all duration-200"
      />
      <button
        @click="send"
        :disabled="disabled"
        class="px-4 sm:px-6 py-2 sm:py-3 bg-primary-600 text-white rounded-lg hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 disabled:bg-gray-400 disabled:cursor-not-allowed font-medium text-sm sm:text-base"
      >
    ...
    <div class="mt-2 text-xs text-gray-500 flex justify-between items-center">
      ...
      <span v-if="input.length > 0" class="text-primary-600">{{ input.length }} символов</span>
    </div>
  </div>
</template>
...
const historyUp = () => {
  ...
  // TypeScript: гарантируем, что значение существует
