			...
			<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
				...
				<div class="lg:col-span-1 space-y-6">
					...
					<div class="bg-white rounded-xl shadow-sm p-6">
						...
						<div class="mt-4 pt-4 border-t">
							...
							<button @click="openDepositModal = true" class="mt-4 w-full btn-primary">Пополнить</button>
						</div>
					</div>
				...
				<div class="lg:col-span-2 space-y-6">
					...
					<div class="bg-white rounded-xl shadow-sm p-6">
						...
						<div v-if="transactions.length === 0" class="text-center py-8 text-gray-500">Пока нет операций</div>
			...
			>
				<div class="bg-white rounded-xl p-6 w-full max-w-md mx-4 animate-scaleIn">
					<div class="flex justify-between items-center mb-4">
						...
						<button @click="openDepositModal = false" class="text-gray-400 hover:text-gray-600">✕</button>
					</div>
					<form @submit.prevent="handleDeposit">
						<div class="space-y-4">
							<div>
								<label class="block text-sm font-medium text-gray-700 mb-1">Сумма (₽)</label>
								<input v-model.number="depositAmount" type="number" min="1" step="100" class="input" required />
...
const loadTransactions = () => {
	...
	if (saved) {
		...
		transactions.value = parsed.map((tx: any) => ({
			...tx,
			date: new Date(tx.date), // преобразуем строку в Date
		}))
	} else {
		transactions.value = [
			{
