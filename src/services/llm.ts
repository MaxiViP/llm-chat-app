	...
	async sendStream(
		...
		if (this.key === 'openrouter') {
			...
			if (limits.perMinute <= 0 || limits.perDay <= 0) {
				throw new Error(`Лимиты исчерпаны: ${limits.perMinute} запросов/мин, ${limits.perDay} запросов/день`)
			}
			this.consumeLimit()
		}

		const { temperature = 0.7, maxTokens } = options
		const finalMaxTokens = this.key === 'openrouter' ? Math.min(maxTokens ?? 1024, 1500) : (maxTokens ?? 8192)
...
function resolveModel(model: string, provider: ProviderKey): string {
	...
	if (alias) {
		...
		if (!resolved) throw new Error(`Алиас модели "${model}" не поддерживается провайдером "${provider}"`)
		return resolved
	}
	return model
}
	...
	for (const modelConfig of models) {
		...
		try {
			...
			if (isResponseValid(response, minLength)) {
				...
				console.warn(`⚠️ Ответ от ${modelConfig.name} слишком короткий (${response?.length || 0} символов)`)
				onChunk('')
